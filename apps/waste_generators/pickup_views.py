...
@login_required(login_url='login')
def edit_pickup_view(request, id):
    pickup_id = id
    Pickups = get_object_or_404(WastePickUp, id=pickup_id)
    waste_source = MasterSource.objects.all()
    pickup_date = WastePickUp.objects.first()
    waste_type = CommodityGroup.objects.all()
    food_type = CommodityMater.objects.all()
    address = Address.objects.all()
    weight= WasteSource.objects.first()
    destination= Collector.objects.all()
...
def update_pickup(request):
    try:
        ...
        # if not pickup_id:
        #     return JsonResponse({"success": False, "message": "Pickup ID is required."})
        ...
        pickup.waste_source = MasterSource.objects.get(id=generator)
        pickup.pickup_date = WastePickUp.objects.get(id=pickup_date)
        pickup.waste_source.waste_type = CommodityGroup.objects.get(id=waste_type)
        pickup.waste_source.food_type = CommodityMater.objects.get(id=food_type)
        pickup.address = Address.objects.get(id=address)
        pickup.waste_source.waste_weight= WasteSource.objects.get(id=waste_weight)
        pickup.destination= Collector.objects.get(id=destination)
        ...
        for record in records:
            ...
            for field, value in record.items():
                if isinstance(value, str):  # Check if the value is a string
                    try:
                        # Try to find the object with the field and its corresponding value
